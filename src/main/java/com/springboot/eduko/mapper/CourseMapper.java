package com.springboot.eduko.mapper;

import com.springboot.eduko.dtos.CourseDto;
import com.springboot.eduko.dtos.RoleDto;
import com.springboot.eduko.model.EduCourses;
import com.springboot.eduko.model.EduRoles;
import org.mapstruct.Mapper;

import java.util.List;

@Mapper(componentModel = "spring")
public interface CourseMapper {
    EduCourses toEntity(CourseDto dto);
    CourseDto toDto(EduCourses entity);
    List<CourseDto> toDto(List<EduCourses> entity);
    List<EduCourses> toEntity(List<CourseDto> dto);
}
